	...
	<section id="content">
		...
		<main>
			...
			<ul class="breadcrumbs">
				...
				<li class="divider">/</li>
				<li><a href="facultydetails.php" class="active">Add Faculty </a></li>
			</ul>
					...
					<form method="post" class="form">
						...
						<div class="content">
							...
							<div class="form-control">
								...
								<input type="text" name="instcode" required id="">
							</div>
							<div class="form-control">
							...
							<input type="email" name="facultymail" id="" required></div>
							<div class="form-control">
		...
		<?php
			...
			$intc = strtoupper($_POST['instcode']);
			$bank = strtoupper($_POST['bankname']);
			$acc = $_POST['accno'];
			$ifsc = strtoupper($_POST['ifsccode']);
			$department = strtoupper($_POST['department']);
			$facultymail = filter_input(INPUT_POST, 'facultymail', FILTER_VALIDATE_EMAIL);
			

			$query = "SELECT *FROM `faculty_details` where faculty_name = upper('$name') and phone = '$phone' or phone = '$phone'";
			 mysqli_query($conn, $query);
			if (mysqli_affected_rows($conn) > 0) {
				echo '<script>swal("Error!", "Data Already Exists", "error").then((value) => {
					location.href="facultydetails.php";
				  });</script>';
				exit;
			}
			//handle phone number exception

			$sql = "INSERT INTO `faculty_details` VALUES(NULL,'$name', '$facultymail','$desig','$department', '$intname','$intc', '$phone','$bank', '$acc', '$ifsc');";
			...
			if(isset($_POST['dbbtn1'])){
				$file =  $_FILES['csvfile']['tmp_name'];
						...
						while(($cont = fgetcsv($handle,1000,",")) !== false)
						{
									...
									$query = "INSERT INTO `faculty_details` VALUES ('', '${cont['0']}', '${cont['1']}', '${cont['2']}', '${cont['3']}', '${cont['4']}', '${cont['5']}')";
